...
function initAuth() {
  ...
  const checkAuthReady = setInterval(() => {
    if (window.firebaseFns) {
      ...
      const gatedSectionIds = ['hero-section', 'about-section', 'crisis-section', 'programs-section', 'stories-section', 'involved-section', 'contact-section'];
      ...
      window.firebaseFns.onAuthStateChanged(window.firebaseAuth, (user) => {
        ...
        if (user) {
          ...
          if (authSignupSection) authSignupSection.style.display = 'none';
          if (authLoginSection) authLoginSection.style.display = 'none';
          if (donateBtn) donateBtn.style.display = '';
          if (floatingCrisisBtn) floatingCrisisBtn.style.display = '';
          gatedNavLinks.forEach(link => { link.style.display = ''; });
          gatedSectionIds.forEach(id => {
            const el = document.getElementById(id);
            if (el) el.style.display = '';
          });
          if (topBanner && welcomeBannerText) {
            welcomeBannerText.textContent = `Welcome back, ${user.displayName || user.email}!`;
          ...
          if (authSignupSection) authSignupSection.style.display = 'none';
          if (authLoginSection) authLoginSection.style.display = '';
          if (donateBtn) donateBtn.style.display = 'none';
          if (floatingCrisisBtn) floatingCrisisBtn.style.display = 'none';
          ...
          if (hashTarget === 'dashboard-section' || gatedSectionIds.includes(hashTarget)) {
            window.location.hash = '#auth-login-section';
          }
        }
      });
...
// ==========================================================================
// Horizontal Page Navigation (one section per "page", scrolled sideways)
// ==========================================================================
...
function initHorizontalNav() {
  ...
  // Convert vertical mouse-wheel input into horizontal scrolling
  mainScroll.addEventListener('wheel', (e) => {
    if (Math.abs(e.deltaY) > Math.abs(e.deltaX)) {
      e.preventDefault();
      mainScroll.scrollLeft += e.deltaY;
    }
  }, { passive: false });

  // Intercept in-page anchor links so they scroll horizontally instead of vertically
  const anchorLinks = document.querySelectorAll('a[href^="#"]');
  anchorLinks.forEach(link => {
    if (link.id === 'link-goto-signup' || link.id === 'link-goto-login') return; // handled separately
  ...
  // Highlight the nav link matching whichever section is currently in view
  const sections = mainScroll.querySelectorAll(':scope > section');
  const navLinks = document.querySelectorAll('.nav-link');

  const setActiveNav = (sectionId) => {
    navLinks.forEach(link => link.classList.remove('active'));
    const match = document.querySelector(`.nav-link[href="#${sectionId}"]`);
...
// ==========================================================================
// Login / Sign Up Toggle (only one shown at a time, before login)
// ==========================================================================

function initAuthToggleLinks() {
  const gotoSignup = document.getElementById('link-goto-signup');
  ...
  if (gotoSignup) {
    gotoSignup.addEventListener('click', (e) => {
      ...
      if (signupSection) {
        ...
        signupSection.scrollIntoView({ behavior: 'smooth', inline: 'start', block: 'nearest' });
      }
    });
  }
  ...
  if (gotoLogin) {
    gotoLogin.addEventListener('click', (e) => {
      ...
      if (loginSection) {
        ...
        loginSection.scrollIntoView({ behavior: 'smooth', inline: 'start', block: 'nearest' });
      }
    });
  }
}
