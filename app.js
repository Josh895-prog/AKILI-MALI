...
// Translation, Form Validation, Modals, and Interactive Elements
// ==========================================================================
...
function updateDOMTranslations() {
  ...
  document.querySelectorAll('[data-i18n]').forEach(el => {
    ...
    if (dict[key]) {
      // If element contains nested HTML/nodes (like <strong> or links), preserve them structure-wise if possible,
      // or fall back to standard text replacement if it is simple.
      if (el.children.length === 0) {
        el.textContent = dict[key];
      } else {
        // Safe mapping for elements with links/anchors (e.g. emergency banner)
        const bannerLink = el.querySelector('.banner-link');
        const normalSpan = el.querySelector('span[data-i18n]');

        if (key === 'emergency_banner_text' && bannerLink) {
          // Special wrapper updates
...
function initNavigation() {
  ...
  // Close nav on anchor link navigation click
  ...
  // Adjust menu offset on scrolling
  const header = document.querySelector('.main-header');
  window.addEventListener('scroll', () => {
    if (window.scrollY > 20) {
      header.style.top = '0';
    } else {
      header.style.top = '45px'; // Matches height of emergency banner
    }
  });
}
...
// ==========================================================================
// Forms Handling & Localized Client Validation
// ==========================================================================
...
function validateKenyanPhone(phone) {
  // Supports formats: +254 712345678, +254 112345678, 0712345678, 0112345678, 254712345678
  // Kenyan prefixes are 07xx or 01xx
